...
> | URL Vercel | https://nexora-membresias.vercel.app |
> | Repo GitHub | https://github.com/Nexora/pack-membresias |
...
1. Contiene **20 prompts secuenciales**: CHAT 1 → CHAT 19. Cada uno se pega en un **chat NUEVO** de Roo Code + DeepSeek, en orden. NO pegues varios en el mismo chat.
...
| Tiempo estimado | 18-28 días de desarrollo |
| Despliegue | Vercel (producción) |
| Fecha de entrega acordada | para el próximo mes |
| Mantenimiento | No incluido (opcional) |
| Asistentes IA (bots) | No incluidos |

### 🎯 Punto de partida según el tipo de servicio

> Este pack no es genérico: está afinado para un proyecto de **Portal de membresías (cobro recurrente y área privada)**. Toda decisión de UX, contenido, datos y QA debe alinearse con esto.

**🎭 Rol que conduce el proyecto:** Consultor de negocios de membresía + arquitecto de cobro recurrente

**🥇 Conversión #1 (el objetivo comercial):** Convertir visitas en MIEMBROS con cobro recurrente: el interesado se registra, elige plan y el sistema cobra solo cada mes — con área privada y reportes de retención.
...
PROYECTO: Portal de membresías (cobro recurrente y área privada) para Gimnasio FitZone · Nivel Avanzado.
TIPO DE PÁGINA: portal de membresías (cobro recurrente y área privada) para gimnasio / entrenador.
...
SERVICIOS/OFERTA A MOSTRAR: gimnasio con entrenamiento funcional.
ESTRUCTURA ACORDADA CON EL CLIENTE: Inicio, Planes, Área de miembros.
NEGOCIO: "Quiero un portal de membresías con cobro recurrente y área de miembros".
...
- Negocio: Portal de membresías (cobro recurrente y área privada) · Giro: Gimnasio / entrenador.
- Servicios/oferta: gimnasio con entrenamiento funcional.
- Descripción del cliente: "Quiero un portal de membresías con cobro recurrente y área de miembros".
...
- 1. Login → Panel → Tarea principal (CRUD/reporte) → Cierre de sesión.
- 2. Fricción típica: login confuso, navegación del módulo, errores sin mensaje claro.
...
Incorpora la estructura acordada con el cliente: "Inicio, Planes, Área de miembros".
...
- **Tarea 1**: El visitante ve los planes y elige uno.
- **Tarea 2**: Se registra y paga la primera cuota (Stripe, cobro recurrente).
- **Tarea 3**: Entra al área privada y ve su plan, pagos y beneficios.
- **Tarea 4**: El admin ve la retención (quiénes se quedan, quiénes se van) y actúa.
...
- **Colores de marca**: 2-4 colores (hex) que usa el negocio (logo, redes, local); si no los hay, propón una paleta coherente con el giro **Gimnasio / entrenador** y márcala como propuesta a validar.
...
- **Mensaje de venta:** Cobra la membresía cada mes de forma automática y retén a tus miembros.
...
### Secciones a construir (Portal de membresías (cobro recurrente y área privada) — 6 bloques)
- / — Home con planes, beneficios y prueba social.
- /planes — Planes de membresía con precios y comparativa.
- /registro y /login — Alta de miembro (correo y/o OAuth).
- /mi-cuenta — Área privada: plan, pagos, datos y beneficios.
...
- gimnasio con entrenamiento funcional — con descripción breve, beneficios y CTA de contacto.

### Sitemap / estructura acordada con el cliente
El cliente describió la estructura así: "Inicio, Planes, Área de miembros". Asegúrate de que la navegación y las secciones reflejen esta estructura de forma completa y coherente.
...
- El perfil de **Google Business Profile** y el plan de reseñas se ejecutan en el CHAT 19/20.
...
Crea `docs/ux/voice-tone.md`: cómo suena la marca según el giro **Gimnasio / entrenador** y el análisis (Cobra la membresía cada mes de forma automática y retén a tus miembros.):
...
### 4. Diseño conversacional de asistentes IA (boceto para asistentes futuros)
Deja en `docs/ux/conversation-design.md` la especificación conversacional que seguirán los asistentes IA si el cliente los agrega después:
...
- **Recuperación de errores**: si no entiende o no sabe, lo dice sin culpar ("No estoy seguro de eso..."), pide clarificación UNA vez y, si persiste la duda, **escala a WhatsApp/humano**.
...
- **Medición conversacional**: la tasa de escalación y los fallbacks se miden en el CHAT 10 (eventos de bot) y se evalúan en el CHAT 14.
...
- `docs/ux/conversation-design.md` (boceto) listo para cuando el cliente agregue asistentes IA.
...
create table if not exists public.suscripciones (
  id uuid primary key default gen_random_uuid(),
  miembro_id uuid  -- FK a la tabla correspondiente (ajustar en kickoff),
...
create table if not exists public.pagos_membresia (
  id uuid primary key default gen_random_uuid(),
  suscripcion_id uuid  -- FK a la tabla correspondiente (ajustar en kickoff),
  proveedor text,
  referencia text,
  monto numeric(10,2),
  estatus text,
...
create table if not exists public.retencion_report (
  id uuid primary key default gen_random_uuid(),
  mes text,
  suscriptores_iniciales text,
  altas text,
  bajas text,
  churn numeric,
  created_at timestamptz not null default now()
...
- **RF-04** · [Alta] Sistema de cuentas de usuario: registro, login (correo y/o OAuth), recuperación de contraseña y sesión segura.
...
- **RF-18** · [Alta] Membresías: cobro recurrente (Stripe), área privada de miembro, gestión de planes y reportes de retención.
...
- 1. El visitante ve los planes y elige uno.
- 2. Se registra y paga la primera cuota (Stripe, cobro recurrente).
- 3. Entra al área privada y ve su plan, pagos y beneficios.
- 4. El admin ve la retención (quiénes se quedan, quiénes se van) y actúa.
...
- Crea **`GET /api/health`** (liveness) que responda `200 { ok: true }` sin depender de servicios; el CHAT 18 lo ampliará con readiness y monitoreo.
- La **auditoría completa de seguridad (OWASP Top 10)** se hace en el **CHAT 15**; aquí deja la base (Zod, rate limiting, /api/health, sin PII en logs, headers básicos) para que esa fase pruebe y endurezca.
...
### Objetivo
- Pirámide de pruebas: muchas unitarias, algunas de integración y pocas E2E (estas van en el CHAT 14).
...
- **Runner**: `vitest` (rápido, TS nativo) con `jsdom`/`happy-dom` para componentes y `node` para lib. Configura `vitest.config.ts` (setup, alias `@/`, coverage) y `npm test` que corra unitarias + integración + componentes (las E2E van por separado en el CHAT 14).
- **Coverage**: `@vitest/coverage-v8` con umbrales mínimos por lógica crítica (p. ej. ≥ 70-80% en `lib/` y en las API routes); el CI/CD (CHAT 13) falla si baja.
...
- **LLM mockeado SIN red**: responde fixtures fijos por caso (nunca llamar a DeepSeek en CI — costo 0 y determinista), igual que en el QA de IA (CHAT 14).
...
- El **CHAT 13** lo conecta al pipeline (corre en cada PR y antes de cada deploy); aquí déjalo listo y documentado.
...
### Objetivo
- Pipeline CI en cada PR: lint, typecheck, unitarias + integración (CHAT 12), E2E (CHAT 14) y auditorías.
...
Crea `.github/workflows/ci.yml` que corra en cada PR y push (y documenta que el CHAT 14 añade sus E2E a este mismo pipeline):
- **Jobs en paralelo** (con `cache` de `node_modules` y `.next` para ir rápido):
  ...
  - `test`: `npm test` (la suite del CHAT 12) con coverage (falla si baja del umbral).
  - `e2e`: `npm run test:e2e` (Playwright, del CHAT 14) contra un build local o preview.
  ...
  - `perf`: Lighthouse CI (del CHAT 16) con umbrales (Performance ≥ 90, CWV en verde) sobre el preview.
...
- **Post-deploy**: verifica `/api/health` y `/api/ready` (del CHAT 18) en el dominio de producción y registra el commit/release.
- **Rollback**: documenta y prueba el **instant rollback de Vercel** (1 clic a un deploy anterior) y deja el procedimiento en el README (se usa en el CHAT 18).
...
- Verifica que el pipeline no imprima secretos en los logs y que `gitleaks` corra en CI (del CHAT 15).
- Rota las keys con el procedimiento documentado en el CHAT 15.
...
- **Jobs programados** (GitHub Actions `schedule` o Vercel Cron):
  - Smoke test diario de la web en producción (visita home + una transacción de prueba → alerta si falla; lo usa el CHAT 18).
  - Verificación de backups (el CHAT 18 define el runbook).
...
> El objetivo de esta fase es que la web **cumpla** esos umbrales y se vea impecable. La **optimización profunda** (Core Web Vitals, imágenes, bundle, caché, servidor/DB y monitoreo) la hace el **CHAT 16** justo después; si aquí algo no llega al umbral, anótalo para el CHAT 16 y sigue (el CHAT 16 corrige la causa raíz).
...
- "Cuentas de usuario (registro/login)" funciona de punta a punta.
- "Base de datos segura" funciona de punta a punta.
...
- **A04 · Insecure Design**: límites de rate y de negocio (intentos de login, subida de archivos con tamaño/tipo, cuotas de uso del LLM del CHAT 14); validación de reglas de negocio (no reservar dos veces la misma cita, no stock negativo).
...
- **Abuso del LLM**: peticiones excesivas a un bot en una sesión → el límite aplica (ya probado en el CHAT 14).
...
- **Métricas clave** (expónlas en `/api/ready` y, si es viable, en `/api/metrics`): p95 latencia y tasa de 5xx por ruta, duración de procesos en segundo plano, llamadas/costo de DeepSeek, uso de Supabase y `uptime` de la DB.
...
- LLM/DeepSeek: tasa de error o timeouts > 10% en 10 min.
...
- La **optimización profunda** (Core Web Vitals, imágenes, bundle, caché, servidor/DB) ya la hizo el **CHAT 16**; aquí solo la sostienes con SLOs, presupuesto en CI y monitoreo de campo.
...
- **RUM**: confirma que el monitoreo real (`web-vitals`/Vercel Analytics) del CHAT 16 captura datos de campo y define una alerta si un Core Web Vital de campo se degrada.
...
- **Anti-spam en formularios**: honeypot oculto, límite por sesión y validación Zod (ya en el CHAT 9); nunca aceptes envíos sin validar.
...
6. Verificar con **Lighthouse/PageSpeed Insights** en producción antes de entregar (los Core Web Vitals deben seguir en verde en el dominio final; el CHAT 16 dejó el baseline y el monitoreo RUM).
...
8. **SEO local y reseñas (motor de crecimiento de un negocio local)**: crea/completa el **Google Business Profile** al 100% (categoría, horario, fotos, servicios, enlace a la web), verifica la **NAP** consistente (nombre/dirección/teléfono idénticos en web, GBP y directorios) y deja el **plan de reseñas** (cómo pedirlas y responderlas) — la ejecución y el reporte de 30-60 días se hacen en el CHAT 20.
...
- Recorre la web sección por sección usando la lista de **criterios de aceptación** del CHAT 14 y el **objetivo comercial #1** definido en el CHAT 1.
...
- **Prioriza 1-2 mejoras** con impacto (no una lista): p. ej. el CTA que no convierte, la página lenta en celular (CWV de campo del CHAT 16), o el contenido que los usuarios no leen.
- **Web performance de campo**: verifica Core Web Vitals reales (Search Console / Vercel Analytics); si LCP sube, aplica lo del CHAT 16.
...
Cuando termines, responde ÚNICAMENTE con el marcador `FIN_DE_FASE_20` + resumen final del pack (URL, aprobación del cliente y primeras decisiones de crecimiento). Con esto el PACK queda COMPLETO: de la investigación al crecimiento.