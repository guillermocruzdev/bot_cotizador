> #️⃣ **REGISTRO · PK-026** — identifica este PACK en Vercel / GitHub
>
> | Campo | Valor |
> |---|---|
> | Código | `PK-026` |
...
| Tiempo estimado | 45-80 días de desarrollo |
...
> Este pack no es genérico: está afinado para un proyecto de **Marketplace con split de pagos (escrow multi-vendedor)**. Toda decisión de UX, contenido, datos y QA debe alinearse con esto.
...
PROYECTO: Marketplace con split de pagos (escrow multi-vendedor) para Plaza Digital MX · Nivel Avanzado.
...
SERVICIOS/OFERTA A MOSTRAR: productos de múltiples vendedores.
ESTRUCTURA ACORDADA CON EL CLIENTE: Inicio, Tiendas, Producto, Panel del vendedor.
NEGOCIO: "Quiero un marketplace donde cada vendedor reciba su parte automáticamente al vender (split de pagos)".
...
- Negocio: Marketplace con split de pagos (escrow multi-vendedor) · Giro: Empresa / operación (plataforma a medida).
- Servicios/oferta: productos de múltiples vendedores.
- Descripción del cliente: "Quiero un marketplace donde cada vendedor reciba su parte automáticamente al vender (split de pagos)".
- Sin referencia de estilo definida.
- Presupuesto: 120000 · Entrega: para el próximo mes.
...
- **Mensaje de venta:** Cada vendedor cobra su parte al instante; tú administras la plataforma y cobras comisión.
...
### Secciones a construir (Marketplace con split de pagos (escrow multi-vendedor) — 8 bloques)
- / — Home con categorías, productos destacados y CTA de vender.
- /productos — Catálogo de todos los vendedores con filtros y búsqueda.
- /productos/[slug] — Detalle de producto (vendedor, fotos, precio, agregar al carrito).
- /carrito y /checkout — Compra con split de pagos entre vendedores.
...
- productos de múltiples vendedores — con descripción breve, beneficios y CTA de contacto.

### Sitemap / estructura acordada con el cliente
El cliente describió la estructura así: "Inicio, Tiendas, Producto, Panel del vendedor". Asegúrate de que la navegación y las secciones reflejen esta estructura de forma completa y coherente.
...
Crea `docs/ux/voice-tone.md`: cómo suena la marca según el giro **Empresa / operación (plataforma a medida)** y el análisis (Cada vendedor cobra su parte al instante; tú administras la plataforma y cobras comisión.):
