...
8. **\`.env.example\` y variables de entorno**: crea \`.env.example\` con TODAS las variables que el proyecto usa (Supabase, DeepSeek, Resend, Stripe, NextAuth... con placeholders y un comentario de qué es cada una) para que el CHAT ${phases.deploy} solo copie y llene. Las reales van en \`.env.local\` (NUNCA se sube) y en Vercel.
...
- Tras el primer \`vercel --prod\` obtienes una URL tipo \`https://<proyecto>.vercel.app\` (y un dominio propio si se configura). **Esa URL ES la entrega**: compártela por WhatsApp/correo con el cliente para la demo del CHAT ${phases.postLaunch}.
