...
- **✅ Cotizador "Alex" (`app/`, `components/`, `lib/` del cotizador):** completo y validado (regresión 319 asserts). Es lo que corre en prod.
...
| `npm run test:regression`                    | Regresión del motor del bot (319 asserts, tsx).                                                               |
...
docs/                           → Documentación (MERCADO_VIBECODER.md, prompts/PACK-*.md)
...
17. **Preguntas consolidadas:** `budget` pregunta fecha+monto juntos; si el cliente da SOLO uno, `nextNode` manda a `clarify_budget` (afina el que falta, máx 2 vía `noSeContador`); el check "ambos ya seteados → `contact_name`" va PRIMERO (aunque la frase diga "no sé cuánto cobran", es retórico si ya dio los dos). `contact_name` extrae los 3 datos con `extractName`+`extractEmail`+`normalizePhone` y deriva a `contact_email`/`contact_phone` solo si falta algo.
