...
- 🤖 **Pack de prompts para Roo Code + DeepSeek (mobile-first, por fases)**: se genera en segundo plano y se descarga como `.txt`. En lugar de un solo documento enorme, es un **pack de 20-26 prompts secuenciales** (estrategia UX e investigación, arquitectura de información + wireframes + flujos, **brand y contenido real — kickoff con el cliente: logo, fotos, textos y testimonios—**, fundación + tokens, shell + componentes UI + patrones de interacción, secciones de contenido, **conversation design y microcopy**, modelo de datos + Supabase, lógica/API/integraciones, analítica en 2 fases (instrumentación + reporting), asistentes IA si aplica, **pruebas unitarias e integración**, **CI/CD (pipeline con lint, typecheck, tests, E2E, auditorías y Lighthouse en cada PR; previews por rama y deploy a Vercel)**, QA web + pulido, **seguridad (OWASP)**, **rendimiento (Performance Engineering: Core Web Vitals, imágenes, bundle, caché y monitoreo)**, accesibilidad/privacidad/IA responsable, **SRE: confiabilidad/observabilidad/operaciones**, despliegue + entrega en Vercel, y **presentación, aprobación y crecimiento post-lanzamiento — UAT con el cliente, SEO local/reseñas y bucle de 30-60 días—**). Cada fase se pega en un **chat nuevo** con su propio contexto compacto para **ahorrar tokens**, y todas construyen la web **primero para celular (360px)** y luego la escalan a tablet/escritorio. Al final (CHAT 20-26) la página queda probada en todos los tamaños, rápida (Core Web Vitals en verde), en cumplimiento, operada (health checks, alertas, respaldos), desplegada al 100% y con el ciclo de presentación/aprobación y crecimiento de 30-60 días (SEO local, reseñas y analítica).
...
> El `prompt_tecnico` (pack de 20-26 chats mobile-first por fases) se genera **siempre** de forma determinista en `lib/prompt-builder.ts` (calidad garantizada), tomando el alcance refinado por DeepSeek cuando la API está configurada, o el catálogo local como respaldo.
